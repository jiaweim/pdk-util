 ...
 * function.
 * <br>
 * The {@link #initGuess(double[])} initial guess values} must be passed
 ...
 * @author Jiawei Mao
 * @version 1.0.0
 * @since 29 Apr 2026, 12:54 PM
 */
public class HarmonicFitter extends CurveFitter {

    public static HarmonicFitter create() {
        return new HarmonicFitter();
    }
    ...
    protected LeastSquaresProblem getProblem(Collection<WeightPoint2D> points) {
        ...
        final double[] startPoint = initialGuess_ != null ?
                initialGuess_ :
                // Compute estimation.
                new ParameterGuesser(points).guess();
    ...
    public static class ParameterGuesser {
        ...
        public ParameterGuesser(Collection<WeightPoint2D> observations) {
            ...
            final WeightPoint2D[] sorted
                    = sortObservations(observations).toArray(new WeightPoint2D[0]);
         ...
         */
        private List<WeightPoint2D> sortObservations(Collection<WeightPoint2D> unsorted) {
            final List<WeightPoint2D> observations = new ArrayList<>(unsorted);

            // Since the samples are almost always already sorted, this
            // method is implemented as an insertion sort that reorders the
            // elements in place. Insertion sort is very efficient in this case.
            WeightPoint2D curr = observations.get(0);
            final int len = observations.size();
            for (int j = 1; j < len; j++) {
                WeightPoint2D prec = curr;
                curr = observations.get(j);
                if (curr.getX() < prec.getX()) {
                    // the current element should be inserted closer to the beginning
                    int i = j - 1;
                    WeightPoint2D mI = observations.get(i);
         ...
         *                                      produce sensible results.
         */
        private double[] guessAOmega(WeightPoint2D[] observations) {
         ...
         */
        private double guessPhi(WeightPoint2D[] observations) {
