...
/**
 * An {@link OutputStream} that writes data to a byte[] and optionally flushes to another {@link OutputStream}.
 * Utility methods are provided for efficiently writing primitive types and strings using big endian.
 * <p>
 * The implementation of this class refers to kryo.
 *
 * @author Jiawei Mao
 * @version 1.0.0
 * @since 17 Apr 2025, 8:58 AM
 */
public class Output extends OutputStream implements AutoCloseable {
    ...
    /**
     * Creates an Output for writing to a byte[].
     *
     ...
     */
    protected boolean require(int required) throws PDKRuntimeException {
        if (capacity - position >= required) return false;
        flush();
        if (capacity - position >= required) return true;
        if (required > maxCapacity - position) {
            if (required > maxCapacity)
                throw new BufferUnderflowException("Buffer overflow. Max capacity: " + maxCapacity + ", required: " + required);
     ...
     */
    public void flush() throws PDKRuntimeException {
     ...
     * Writes a byte.
     */
    public void write(int value) throws PDKRuntimeException {
     ...
     */
    public void write(byte[] bytes) throws PDKRuntimeException {
     ...
     * Writes the bytes. Note the number of bytes is not written.
     */
    public void write(byte[] bytes, int offset, int length) throws PDKRuntimeException {
        writeBytes(bytes, offset, length);
    }

    // byte:

    public void writeByte(byte value) throws PDKRuntimeException {
        if (position == capacity) require(1);
        buffer[position++] = value;
    }

    public void writeByte(int value) throws PDKRuntimeException {
        if (position == capacity) require(1);
        buffer[position++] = (byte) value;
    }
     ...
     * Writes the bytes. Note the number of bytes is not written.
     */
    public void writeBytes(byte[] bytes) throws PDKRuntimeException {
     ...
     */
    public int writeInt(int value, boolean optimizePositive) throws PDKRuntimeException {
     ...
     */
    public int writeVarInt(int value, boolean optimizePositive) throws PDKRuntimeException {
     ...
     * @return The number of bytes written.
     */
    public int writeVarIntFlag(boolean flag, int value, boolean optimizePositive) throws PDKRuntimeException {
        if (!optimizePositive) value = (value << 1) ^ (value >> 31);
     ...
     */
    public void writeLong(long value) throws PDKRuntimeException {
        require(8);
        byte[] buffer = this.buffer;
        int p = position;
        position = p + 8;
     ...
     */
    public int writeLong(long value, boolean optimizePositive) throws PDKRuntimeException {
     ...
     */
    public int writeVarLong(long value, boolean optimizePositive) throws PDKRuntimeException {
     ...
     */
    public void writeFloat(float value) throws PDKRuntimeException {
        require(4);
        byte[] buffer = this.buffer;
        int p = position;
        position = p + 4;
     ...
     * @return The number of bytes written.
     */
    public int writeVarFloat(float value, float precision, boolean optimizePositive) throws PDKRuntimeException {
     ...
     */
    public void writeDouble(double value) throws PDKRuntimeException {
        require(8);
        byte[] buffer = this.buffer;
        int p = position;
        position = p + 8;
     ...
     * @return The number of bytes written.
     */
    public int writeVarDouble(double value, double precision, boolean optimizePositive) throws PDKRuntimeException {
     ...
     */
    public void writeShort(int value) throws PDKRuntimeException {
        require(2);
        int p = position;
        position = p + 2;
        buffer[p] = (byte) value;
     ...
     */
    public void writeChar(char value) throws PDKRuntimeException {
        require(2);
        int p = position;
        position = p + 2;
        buffer[p] = (byte) value;
     ...
     */
    public void writeBoolean(boolean value) throws PDKRuntimeException {
        if (position == capacity) require(1);
     ...
     * are written as UTF8.
     * <p>
     * If a string is known to be ASCII, {@link #writeAscii(String)} may be used. The string can be read using
     * {@link Input#readString()} or {@link Input#readStringBuilder()}.
     *
     * @param value May be null.
     */
    public void writeString(String value) throws PDKRuntimeException {
        if (value == null) {
        ...
        if (charCount > 1 && charCount <= 32) {
            for (int i = 0; i < charCount; i++)
                if (value.charAt(i) > 127) break outer; // not ASCII
...
//                System.arraycopy(bytes, 0, buffer, position, bytes.length);
     ...
     * @param value May be null.
     */
    public void writeAscii(String value) throws PDKRuntimeException {
        if (value == null) {
            writeByte(0x80); // 0 means null, bit 8 means UTF8.
            return;
        }
    ...
    private void writeAscii_slow(String value, int charCount) throws PDKRuntimeException {
     ...
     */
    public void writeInts(int[] array, int offset, int count) throws PDKRuntimeException {
        if (capacity >= count << 2) {
            require(count << 2);
            byte[] buffer = this.buffer;
            int p = position;
     ...
     */
    public void writeInts(int[] array, int offset, int count, boolean optimizePositive) throws PDKRuntimeException {
        if (varEncoding) {
            for (int n = offset + count; offset < n; offset++)
     ...
     */
    public void writeLongs(long[] array, int offset, int count) throws PDKRuntimeException {
        if (capacity >= count << 3) {
            require(count << 3);
            byte[] buffer = this.buffer;
            int p = position;
     ...
     * {@link #setVariableLengthEncoding(boolean)}. This may be more efficient than writing them individually.
     */
    public void writeLongs(long[] array, int offset, int count, boolean optimizePositive) throws PDKRuntimeException {
        if (varEncoding) {
            for (int n = offset + count; offset < n; offset++)
     ...
     */
    public void writeFloats(float[] array, int offset, int count) throws PDKRuntimeException {
        if (capacity >= count << 2) {
            require(count << 2);
            byte[] buffer = this.buffer;
            int p = position;
     ...
     */
    public void writeDoubles(double[] array, int offset, int count) throws PDKRuntimeException {
        if (capacity >= count << 3) {
            require(count << 3);
            byte[] buffer = this.buffer;
            int p = position;
     ...
     */
    public void writeShorts(short[] array, int offset, int count) throws PDKRuntimeException {
        if (capacity >= count << 1) {
            require(count << 1);
            byte[] buffer = this.buffer;
            int p = position;
     ...
     */
    public void writeChars(char[] array, int offset, int count) throws PDKRuntimeException {
        if (capacity >= count << 1) {
            require(count << 1);
            byte[] buffer = this.buffer;
            int p = position;
     ...
     */
    public void writeBooleans(boolean[] array, int offset, int count) throws PDKRuntimeException {
        if (capacity >= count) {
            require(count);
            byte[] buffer = this.buffer;
            int p = position;
